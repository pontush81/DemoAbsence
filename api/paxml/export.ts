...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (deviations.length <= 1) {
      throw new Error('🚫 CRITICAL: PAXML export found insufficient data. Mock fallback is disabled for security.');
    }
    ...
    if (deviations.length === 0) {
      throw new Error('No valid deviations found for export after filtering');
    }
